...
export async function* sync(latestMessage, settings, logger) {
    ...
    try {
        while(true) {
            ...
            yield data = (await makeRequest(settings.token, settings.pairs, latest, logger, settings.fromTimestamp)).map(t => JSON.stringify(t));
            if(data.length) {
...
async function makeRequest(token: string, pairs: string[], latest, logger, fromTimestamp) {
    let timestamp_gt = fromTimestamp || 0;
    const lastObject = latest ? JSON.parse(String(latest)) : '';

    if (latest && lastObject.transaction.timestamp > timestamp_gt) {
    ...
    if(latest) {
        ...
        tempReceived.data.swaps = tempReceived.data.swaps.filter(t => t.transaction.timestamp > lastObject.transaction.timestamp && t.transaction.id != lastObject.transaction.id);
