...
export class telegramBot {
    ...
    amountPubs: number;
    constructor(channel: string, logger = {}, amountPubs = 0, markdownEnable = true) {
    ...
    async sendMessage(message: string) {
        ...
        try {
            if(message.length > 4096) { // Telegram limitation on message length
                ...
                await this.bot.sendMessage(this.channel, message, { parse_mode: this.markdownEnable ?  "MarkdownV2" : undefined, disable_web_page_preview: true})
            }
        } catch(err: any) {
            if(err.response.statusCode == 429) { // Too many requests (We went beyond the telegram limit)
                ...
                await this.bot.sendMessage(this.channel, message, { parse_mode: this.markdownEnable ?  "MarkdownV2" : undefined, disable_web_page_preview: true}); // retry
            } else {
