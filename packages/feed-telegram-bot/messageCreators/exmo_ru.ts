...
export function createMessage(options: Message, constants) {
    ...
    +   `${options.type == "buy" ? "Покупка" : "Продажа"} *${numWithCommas(Math.ceil(+options.quantity))} ${constants.token}* за *${ options.anotherPrice ?  (+options.amount).toFixed(6) : numWithCommas(Math.ceil(+options.amount))} ${symbol}* на EXMO\n\n`
